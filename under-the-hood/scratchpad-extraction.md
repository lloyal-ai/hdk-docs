...
Recovery extraction is what makes the reasoning.run **recon stage** work end-to-end:
...
Tool result compression (the fork-attend-extract-prune pattern below) doesn't currently have App-protocol integration — Apps' tools return whatever the tool wants; if a tool returns a large payload the agent has to live with it in KV unless the harness wraps the call. Future App-side compression hooks are a roadmap item.
