...
`agent.currentTool` is used by `shouldExit` to protect agents mid-generation of the terminal tool from time-budget kills. See [Agent Policy — shouldExit](/under-the-hood/agent-policy#shouldexit--pre-generate-kill-gate).
