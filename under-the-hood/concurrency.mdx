...
The tick loop is the substrate every AgentApp's tools execute on. Two AgentApp-protocol interactions to know up front:

- **The spine is rendered from the AgentApp registry.** Pool setup calls `renderSpine({ apps: registry.enabled() })` and the resulting catalog + tool schemas decode into the spine branch once. The enabled-AgentApp set at the moment of pool creation is what every spawned agent inherits via prefix-share; mid-pool registry changes don't propagate.
- **DISPATCH gates protected tools via `authGuard` before `Tool.execute`.** Phase 4 looks up the tool by name, checks `tool.protected` against the session's `GrantStoreCtx`, denies if no grant exists (emits `tool:authReject` with agent lineage), and only then calls `execute`. Open tools (the default) bypass the check. See [AgentApp security](/build-an-app/app-security).
- **Per-AgentApp scope lifecycles are independent of the pool.** The registry runs each AgentApp's `AppFactory` in a detached Effection scope. AgentApp teardown via `ensure(...)` fires on registry scope exit, not on pool exit — so an AgentApp's external allocations survive across many pool invocations within a single session.

The five-phase tick mechanics below describe the per-pool execution loop. The AgentApp protocol sits on top of it; it doesn't change the phase ordering, but it gates one of the dispatch checks and supplies the spine content.

## Tick Loop

The agent pool runs a five-phase tick loop. Phase 0 drains orchestrator-queued work (new agent spawns and spine extensions); the remaining four phases drive per-agent decode and tool dispatch:
...
PHASE 4 — DISPATCH      execute collected tool calls (currently sequential)
```
...
**Phase 4 (DISPATCH)** executes collected tool calls. Before each tool, the pool creates a fresh `ContextPressure` snapshot (reflecting KV consumed by SETTLE) and evaluates `policy.shouldExplore(agent, pressure)` to determine explore vs exploit mode. The `explore` flag and `pressurePercentAvailable` are passed to the tool via `ToolContext`. Each tool runs inside `scoped()` + `call()`:
