...
The "common prefix" in the App-protocol era is the rendered spine: `renderSpine({ apps })` produces a string containing the framework intro, every enabled App's `CATALOG_ENTRY`, and the tool-selection rule — plus the tool schemas of every enabled App's tools embedded by the chat template. That whole block is what gets decoded **once** into the spine branch and shared via metadata-only fork by every spawned agent in the pool.

What that means for App authors and harness authors:

- **Adding or removing an App via the registry changes what gets prefix-shared** on the *next* pool. Within an active pool, the prefix is fixed at the moment the spine was created.
- **Each App's tools cost roughly `(schema-size × 1)`, not `× N agents`.** A 700-token tool schema is prefilled once at spine setup; the N agents in the pool inherit it via `Branch.forkSync`'s O(1) metadata tag, not by re-tokenizing.
- **Per-spawn suffixes are thin** — just `BOUNDARY_MARKER(app.protocol.name)` + the App's `skill.eta` + user task. Whatever the App's skill template renders to is the per-agent unique cost.

See [What is an App](/build-an-app/what-is-an-app) for the catalog assembly and [Mixed-role pools](/under-the-hood/mixed-role-pools) for the convention this enables for pools running multiple App roles concurrently.

## How `withSpine` + `useAgentPool` works
...
The spine prefills the system prompt and tool schemas once. Each agent forks from the root and prefills only its unique suffix — the user message and generation prompt. The system+tools tokens are never re-decoded.

## KV cache: physical sharing, not copying

When `forkSync()` calls `llama_kv_self_seq_cp()` under the hood, it does not copy KV tensors. It tags existing KV cells (which hold the spine's key/value vectors) with a new sequence ID. Multiple branches can read the same physical memory at positions 0..N-1 while writing their own unique cells at positions N and beyond.
...
After `withSpine` prefills 866 tokens, `root->position = 866`. When an agent calls `forkSync()`, the child branch starts at position 866 — not 0.
...
The suffix tokens are placed at positions 866, 867, 868, etc. The GPU attends to prior KV entries at positions 0-865 (already in cache from the spine) and computes new KV entries only for the suffix positions. The spine's tokens are never re-processed.

## Why `setupAgent` formats the full message array

<Note>
Looking at `setupAgent` in `agent-pool.ts`, the agent calls `formatChatSync` with `[{system, systemPrompt}, {user, content}]` — which includes the system prompt and tools even though the spine already has them. This looks like duplication, but it's intentional:
...
The spine's contribution is visible in trace events:
...
The suffix `tokenCount` includes the spine content (for format detection), but the `branch:prefill` event shows the actual GPU decode cost:
...
Research tools spawn inner agent pools via `withSpine` inside their `execute()` method. The inner spine creates a new branch at position 0, independent of the outer pool. Inner agents fork from the inner spine and share its prefix.

```
Outer pool:
  withSpine (outer spine, pos 0..866)
    ├── Agent A0 (fork at 866, suffix 866..896)
    │     └── [tool dispatch: research]
    │           └── withSpine (inner spine, fork from A0 + 2 token separator)
    │                 ├── Agent A0.0 (fork from inner spine)
    │                 ├── Agent A0.1 (fork from inner spine)
    │                 └── Agent A0.2 (fork from inner spine)
    ├── Agent A1 (fork at 866, suffix 866..903)
    └── Agent A2 (fork at 866, suffix 866..895)
```

Inner and outer pools share the same `BranchStore` and the same physical KV cache. `ContextPressure` snapshots `cells_used` across all branches to enforce headroom limits. See [Concurrency Model](/under-the-hood/concurrency) for details on how nested pools interact with KV pressure.

### Warm path: fork from parent

When `withSpine` receives a `parent` branch option, it forks from the parent instead of creating a new root at position 0. The parent already has the system prompt in its KV, so the inner spine only needs to prefill the turn separator — **2 tokens instead of ~760**.

```typescript
yield* withSpine(
  { systemPrompt, tools, parent: context.branch },
  function*(spine) {
    // Inner agents fork from spine, which forked from the calling agent.
    // They inherit the calling agent's full KV state:
    // system prompt + tool schemas + all prior tool calls + all tool results.
  },
);
```

This is how [Continuous Context](/start-here/what-is-the-hdk#continuous-context-agents-agents-branch-inherit-and-build-on-each-others-attention-state) works for recursive tools. Sub-agents attend over the same physical key-value pairs at the parent's shared positions — no information bottleneck, no re-encoding.

```
Without warm path: inner spine decodes ~760 tokens (full system + tool schemas)
With warm path:    inner spine prefills 2 tokens (turn separator only)
Savings:           ~758 tokens per recursive fork
```

## `cells_used` accounting

`cells_used` is incremented on every decode operation (`decode_each`, `decode_scatter`) and decremented on branch prune. After the inner `withSpine` exits and its root is pruned, `cells_used` drops by the inner spine's unique cells — freeing headroom for the outer pool's remaining agents.

The spine's cells are not freed until the outer `withSpine` exits. This is correct — they're still needed by any surviving outer agents that forked from the root.
...
Corpus pool (5 tools, 3 agents):
  Spine:  866 tokens decoded once (system + 5 tool schemas)
...
Web pool (4 tools, 3 agents):
  Spine:  ~700 tokens decoded once (system + 4 tool schemas)
...
The deeper an agent's trajectory, the larger the savings. Agent 5 (7 turns, 5,607 tokens of tool results) sees a 4.8× reduction. A single-turn agent sees minimal savings — the overhead is the same spine decode either way.
