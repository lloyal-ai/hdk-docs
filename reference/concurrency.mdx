...
When `withSpine` is given a `systemPrompt` + `toolsJson`, it formats the chat header once at setup, prefills it onto the root, and stashes the resulting `FormatConfig` (parser, grammar, format, triggers) on the `SpineFmt` Effection context:
...
*execute(args: { questions: string[] }, context?: ToolContext): Operation<unknown> {
  ...
  return yield* withSpine(
    { systemPrompt, toolsJson, parent: context?.branch },
    //                          ^^^^^^^^^^^^^^^^^^^^^^
...
const delegate = new DelegateTool({
  ...
  poolOpts: { tools: [...baseTools, /* nested delegate added below */], terminalTool: 'report' },
});
