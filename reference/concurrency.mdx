...
`agent.currentTool` is used by `shouldExit` to protect agents mid-generation of the terminal tool from time-budget kills. See [Agent Policy — shouldExit](/reference/agent-policy#shouldexit--pre-generate-kill-gate).
...
The decision boundary depends on the model choosing to continue exploring rather than terminating. This choice is influenced by the ordering of tools in the `tools` array — see [Grammar & Tool Ordering](/reference/grammar-and-ordering#tool-ordering-sensitivity) for the full mechanism and design rule.
...
2. **PRODUCE hard-cut** — agents are killed before `produceSync()` when remaining drops below `hardLimit`. The kill reserves the `hardLimit` cells for `recoverInline` to extract findings via the eager-report grammar. Terminal-tool protection (`agent.currentTool === terminalTool`) yields to critical — mid-report agents get killed if they push past the threshold, and recovery takes over with a budget-aware prompt.
...
See [Playbooks](/reference/playbooks) for the prompt convention this enables.
...
const delegate = new DelegateTool({
  ...
  poolOpts: { tools: [...baseTools, /* nested delegate added below */], terminalToolName: 'report' },
});
...
All trajectories in this section were recorded with `[search, read_file, grep, report, research]` ordering. With `report` placed last (`[search, read_file, grep, research, report]`), step-4 hypothesis greps drop to 0-1/3 agents and the scatter pattern reappears. See [Grammar & Tool Ordering](/reference/grammar-and-ordering#tool-ordering-sensitivity) for the mechanism and the design rule.
