...
When `withSharedRoot` is given a `systemPrompt` + `toolsJson`, it formats the chat header once at setup, prefills it onto the root, and stashes the resulting `FormatConfig` (parser, grammar, format, triggers) on the `RootFmt` Effection context:

```typescript
export const RootFmt = createContext<FormatConfig | null>('lloyal.rootFmt', null);
```

`setupAgent` probes `RootFmt` when initializing each per-spec agent. If it's set, the agent skips re-emitting tool schemas in its own suffix — they're already in attention via metadata-only KV prefix-share — and inherits the parser/grammar/format/triggers directly from the root setup. The agent's per-spec `systemPrompt` formats as `[system, user]` (a second system in lineage; the model handles role shifts), tokenizes onto the agent's branch as the suffix.

When `RootFmt` is unset (`withSharedRoot` called without `systemPrompt`), each agent emits its own tools in its own suffix as in earlier versions of the framework — the pool runs in "non-shared mode" and `extendRoot` writes to `warmParent` (the harness-supplied root) rather than the inner withSharedRoot's root. This is the right default for chains that need spine extensions to persist past pool exit.
...
The inner `withSharedRoot` forks from the calling agent's branch (warm path) or creates a fresh root (cold path). Inner agents inherit the calling agent's full attention state — every prior tool call, every tool result, every token generated. The inner `useAgentPool` runs the same five-phase tick loop as the outer pool. Inner agents share the same `BranchStore` and compete for the same KV cache slots.
...
When `parent` is provided, the shared root forks from the parent branch and prefills only the turn separator — **2 tokens** instead of ~760 for the full system prompt. The system prompt is already in the parent's KV. Sub-agents attend over the same physical key-value pairs at shared positions.
