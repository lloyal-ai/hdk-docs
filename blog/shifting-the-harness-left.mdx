...
<Card title="This run is public" icon="github" href="https://github.com/lloyal-ai/reasoning-run" horizontal>
  The trace, config, and full report are in the repo — run the same swarm with `npx reasoning.run`.
</Card>
...
<Frame caption="One board, N pads, one dispatch — the model inside the harness process.">
  <img
    src="/assets/blog/architecture-shift-left.png"
    alt="Architecture diagram showing one GLM-5.2 model process, one shared KV context board, and ten agent pads dispatched in a single llama_decode call."
  />
</Frame>
...
<Frame caption="Ten agents live on one deployment — the wide capture from the run.">
  <img
...
<CardGroup cols={2}>
  <Card
    title="Run the swarm yourself"
    icon="github"
    href="https://github.com/lloyal-ai/reasoning-run"
  >
    `npx reasoning.run` gets you running. The full trace, config, and cost report from this run are in the repo.
  </Card>
  <Card
    title="Install from npm"
    icon="npm"
    href="https://www.npmjs.com/package/reasoning.run"
  >
    The `reasoning.run` package, published and versioned.
  </Card>
</CardGroup>
